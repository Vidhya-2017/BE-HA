...
if(mysqli_num_rows($candidate_list_result) > 0){
    while ($candidate_listrow = mysqli_fetch_array($candidate_list_result)){
           ...
           $squad_feedback_query       =   "SELECT A.sprintLevel,A.feedbackTxt,A.sq_final_status,B.technical_skill,B.logical_skill,B.communication_skill FROM squad_feedback as A,squad_feedback_skill as B WHERE A.sid= B.squad_fb_id AND A.eventId='$event_ID' AND A.candidate_id = '".$candidate_listrow['CanidateID']."' ";
           ...
           $squad_feedback_lists=[];
            if(mysqli_num_rows($squad_feedback_list_result) > 0){
                while ($candidate_listsfbrow = mysqli_fetch_array($squad_feedback_list_result)){
                    ...
                    $squad_feedback_list['technical_skill']=$candidate_listsfbrow['technical_skill'];
                    $squad_feedback_list['logical_skill']=$candidate_listsfbrow['logical_skill'];
                    $squad_feedback_list['communication_skill']=$candidate_listsfbrow['communication_skill'];                    
...
$squad_feedback_query="SELECT A.sprintLevel,A.feedbackTxt,A.sq_final_status,B.technical_skill,B.logical_skill,B.communication_skill FROM squad_feedback as A,squad_feedback_skill as B WHERE A.sid= B.squad_fb_id AND A.eventId='$event_ID'";
$squad_feedback_list_result =   mysqli_query($conn,$squad_feedback_query);
    $squad_feedback_list = [];
