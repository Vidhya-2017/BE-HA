...
$queryUser = "SELECT EventID as EventId , EventName as Name ,Location ,Duration ,Client ,Date as EventDate, Skills ,AssessmentScale, IF(ProblemSolvingSkillTested=1,'TRUE','FALSE') as PSSkillTest ,IF(TechnicalSkilslsTested=1,'TRUE','FALSE') as TechSkillTested , IF(CommunicationSkillIsTested=1,'TRUE','FALSE') as ComSkillTested,IF(LogicalSkillsIsTested=1,'TRUE','FALSE') as LogSkillTested FROM `register_event` where isClosed='0' and CreatedBy = ".$user_ID." ";
...
if(mysqli_num_rows(@$result) == 0){
  ...
  $queryPanel = "SELECT e.EventID as EventId , e.EventName as Name ,Location ,Duration ,Client ,e.Date as EventDate, e.Skills ,e.AssessmentScale, IF(e.ProblemSolvingSkillTested=1,'TRUE','FALSE') as PSSkillTest ,IF(e.TechnicalSkilslsTested=1,'TRUE','FALSE') as TechSkillTested , IF(e.CommunicationSkillIsTested=1,'TRUE','FALSE') as ComSkillTested,IF(e.LogicalSkillsIsTested=1,'TRUE','FALSE') as LogSkillTested FROM `register_event` e INNER JOIN event_panel_list p ON p.pl_eventId = e.EventID where e.isClosed='0' and p.pl_userid = ".$user_ID." ";
...
if(mysqli_num_rows(@$result) == 0){
  ...
  $queryOrganiser = "SELECT e.EventID as EventId , e.EventName as Name ,Location ,Duration ,Client ,e.Date as EventDate, e.Skills ,e.AssessmentScale, IF(e.ProblemSolvingSkillTested=1,'TRUE','FALSE') as PSSkillTest ,IF(e.TechnicalSkilslsTested=1,'TRUE','FALSE') as TechSkillTested , IF(e.CommunicationSkillIsTested=1,'TRUE','FALSE') as ComSkillTested,IF(e.LogicalSkillsIsTested=1,'TRUE','FALSE') as LogSkillTested FROM `register_event` e INNER JOIN event_organizer o ON o.orgnizer_eventid = e.EventID where e.isClosed='0' and o.organizer_userid = ".$user_ID." ";
...
if(mysqli_num_rows($result) > 0){
    while ($durationrow = mysqli_fetch_assoc($result)){
       ...
       // $sqlOrganisers = "SELECT o.`orgnizer_eventid`, u.`first_name`, u.`last_name`, u.`sapID` FROM `event_organizer` o inner join `user_login` u on u.user_id = o.organizer_userid WHERE o.`orgnizer_eventid` = $event_id";
