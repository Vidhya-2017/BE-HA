...
if(mysqli_num_rows($candidate_list_result) > 0){
    while ($candidate_listrow = mysqli_fetch_array($candidate_list_result)){
           ...
           $fbgrpuser =  mysqli_query($conn,"SELECT ulg.user_id,ulg.first_name,ulg.last_name,ulg.email,ulg.contact_no FROM squad_feedback sfb INNER JOIN user_login ulg ON sfb.createdBy = ulg.user_id where sfb.candidate_id=".$candidate_listrow['CanidateID']." and sfb.eventId=".$candidate_listrow['EventID']." and sfb.isActive=1 group by sfb.createdBy");
           if(mysqli_num_rows($fbgrpuser) > 0){ 
            while ($canUserRow = mysqli_fetch_assoc($fbgrpuser)){               
                   
                 $squad_feedback_query  = "SELECT A.sid as sids,A.sprintLevel,A.feedbackTxt,A.competancy_rating,A.sq_final_status FROM squad_feedback as A WHERE  A.eventId='$event_ID' AND A.createdBy=".$canUserRow['user_id']." AND A.candidate_id = '".$candidate_listrow['CanidateID']."'";
                ...
                if(mysqli_num_rows($squad_feedback_list_result) > 0){
                    while ($candidate_listsfbrow = mysqli_fetch_assoc($squad_feedback_list_result)){
                        ...
                        $othskils=[];
                          ...
                          $candidate_listsfbrow['AssesmentParam']= $othskill; 
                        $squad_feedback_list[]=$candidate_listsfbrow ;
                        array_push($squad_feedback_lists,$squad_feedback_list);


                    }
                    $canUserRow['Feedback']=$squad_feedback_lists;
                    $candidate_list['PanelList'][]=$canUserRow; 
                }
                /* $squad_feedback_query  = "SELECT A.sid as sids,A.sprintLevel,A.feedbackTxt,A.sq_final_status FROM squad_feedback as A WHERE  A.eventId='$event_ID' AND A.createdBy=".$canUserRow['user_id']." AND A.candidate_id = '".$candidate_listrow['CanidateID']."'";
                $squad_feedback_list_result =   mysqli_query($conn,$squad_feedback_query);
                $candidate_list_feedback = [];
                $squad_feedback_lists=[];
                if(mysqli_num_rows($squad_feedback_list_result) > 0){
                    ...
                    print_r($squad_feedback_lists);
                    
                    $candidate_list['PanelList']['feedback'] = $squad_feedback_lists; 
...
$squad_feedback_query="SELECT A.sid as sids, A.sprintLevel,A.feedbackTxt,A.competancy_rating,,A.sq_final_status FROM squad_feedback as A WHERE A.eventId='$event_ID'";
$squad_feedback_list_result =   mysqli_query($conn,$squad_feedback_query);
    $squad_feedback_list = [];
