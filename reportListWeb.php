...
if(mysqli_num_rows($result) > 0){
    while ($hackathon_detailsrow = mysqli_fetch_assoc($result)){
        if (!is_null($hackathon_detailsrow["EventID"])){
...
/**PANEL LIST :: START **/
$panel_query="SELECT a.pl_userid as panel_userid ,a.pl_eventId as panel_eventid, b.first_name,b.last_name ,b.contact_no,b.email FROM event_panel_list a INNER JOIN user_login b  on 	a.pl_userid = b.user_id WHERE b.isActive=1 and a.isActive=1 and a.pl_eventId ='$event_ID' ";
...
//,B.candidate_image
 $candidate_query="SELECT A.candidate_id,A.eventId, A.squad_id,B.EmpName,B.EmailId,B.ContactNo,B.Expereince,B.RelevantExperience FROM squad_feedback as A, candidate_registration as B WHERE A.eventId='$event_ID' AND A.candidate_id=B.ID AND A.isActive=1 AND B.isActive=1 GROUP BY A.candidate_id" ;
...
if(mysqli_num_rows($candidate_list_result) > 0){
    while ($candidate_listrow = mysqli_fetch_assoc($candidate_list_result)){
        ...
        $squad =   mysqli_query($conn,"SELECT D.SquadName,C.squad_id FROM squad_feedback C INNER JOIN squad D ON C.squad_id = D.ID WHERE C.candidate_id='".$candidate_listrow['candidate_id']."'  AND C.eventId='$event_ID' group by C.candidate_id ");
        ...
        /**FEEDBACK DETAIL **/
        $squad_group  = "SELECT sq1.sid as sids,sq1.sprintLevel as spLevel FROM squad_feedback as sq1  WHERE  sq1.eventId='$event_ID' AND sq1.candidate_id = '".$candidate_listrow['candidate_id']."' group by sq1.sprintLevel";
        ...
        if(mysqli_num_rows($squad_group_res) > 0){
            while ($squad_group_row = mysqli_fetch_assoc($squad_group_res)){
                $squad_feedback_query  = "SELECT sq.sid as sids,sq.sprintLevel,sq.feedbackTxt,sq.sq_final_status,sq.role,sq.competancy_rating,ul.first_name,ul.last_name,ul.email,ul.contact_no FROM squad_feedback as sq , user_login as ul WHERE  sq.eventId='$event_ID' AND sq.candidate_id = '".$candidate_listrow['candidate_id']."' AND  sq.createdBy=ul.user_id AND sq.sprintLevel='".$squad_group_row['spLevel']."'";
...
echo $result = json_encode(array("errCode"=>$final_errorcode,"status"=>$final_status,"Hackathon_Details" => $hackathon_details,"Organizers_list" => $organizers_list,"Panels_list"=>$panels_list,"candidate_list"=> $candidate_lists));
